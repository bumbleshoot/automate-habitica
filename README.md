...
Automatically hatches [pets](https://habitica.fandom.com/wiki/Pets), but only if the player has enough [eggs](https://habitica.fandom.com/wiki/Eggs) for all unowned pets/[mounts](https://habitica.fandom.com/wiki/Mounts) of that species, and enough [hatching potions](https://habitica.fandom.com/wiki/Hatching_Potions) for all unowned pets/mounts of that color. Automatically feeds pets, but only if the above conditions are met, *and* the player has enough of the pet's [favorite food(s)](https://habitica.fandom.com/wiki/Food_Preferences) to obtain all unowned mounts with the same favorite food(s). In other words, the script will not choose which pets to hatch/feed for you; it will only hatch/feed pets if you have enough eggs/potions/food for ALL the unowned options! If a choice needs to be made, then the choice is left up to you.
