  ...
  try {
    ...
    if (getUser(true).data.stats.lvl >= 11) {
      ...
      if (saveMana) {
        ...
        log("Reserving no more than " + maxManaAfterCron + " (maxManaAfterCron) + " + stealthMana + " (stealthMana) = " + reserve + " mana");
      ...
      if (user.data.stats.lvl >= 13) {
        ...
        log("Casting Tools of the Trade " + numTools + " time(s)");
      ...
      // if lvl == 12, cast backstab
        ...
        log("Player level " + user.data.stats.lvl + ", casting Backstab " + numBackstabs + " time(s)");
      ...
      // if lvl < 12, cast pickpocket
        ...
        log("Player level " + user.data.stats.lvl + ", casting Pickpocket " + numPickpockets + " time(s)");
...
function castStealthAndDumpMana() {
  try {
    ...
    log("Casting Stealth " + numStealths + " time(s)");
...
function numStealthsNeeded() {
  ...
  if (getUser(true).data.stats.lvl >= 14) {
    ...
    log("Damaging dailies: " + numDamagingDailies);
  ...
  // if lvl < 14, return 0
    ...
    log("Player lvl " + user.data.stats.lvl + ", cannot cast Stealth");
    return 0;
  }
}